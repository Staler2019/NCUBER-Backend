...
def insert_person():
    person = PERSON("0912345678", 12, "108502000", "NewStudent", 1, 502, 4)
        ...
        if clientId == CLIENT_ID:
            ...
            if data["type"] == "req_nums_of_cars":
                ...
                for i in range(1, numbers + 1):
                    ...
                    cars.append(car)
            response = {"type": "req_nums_of_cars", "cars": cars}
    ...
    if request.method == "POST":
        ...
        if clientId == CLIENT_ID:
            ...
            if people_num < car.persons_num_limit or user.gender == car.gender_limit:
                ...
                status = 2
            try:
                response = {"type": "add_person_to_car", "status": status}
...
def rmPersonFromCar():
    if request.method == "POST":
        ...
        if clientId == CLIENT_ID:
            data = request.json
            user = PERSON.query.filter_by(student_id=data["stuId"]).first()
            del_carpool_by_stuid = CARPOOL.query.filter_by(
            ...
            if user.now_carId == data["carId"]:
                ...
                db.session.commit()
                carpool_for_current_carId = CARPOOL.query.filter_by(
                    carId=data["carId"]
                ).first()
                ...
                status = 2
            try:
                response = {"type": "rm_person_from_car", "status": status}
